...
class TestImage(TestCase):
    def setUp(self):
        ...
        self.category = Category(name='wall')
        self.category.save_category()
        self.image_test = Image(id=1, name='wall', description='good', location=self.location,category=self.category)
    ...
    def test_get_image_by_id(self):
        self.image_test.get_image_by_id(self.image_test.id)
        image = Image.objects.filter(id=self.image_test.id)
        self.assertTrue(len(image) == 0)

    def test_search_image_by_location(self):
        self.image_test.save_image()
        found_images = self.image_test.filter_by_location(location='Egypt')
...
class TestLocation(TestCase):
    ...
    def test_update_location(self):
        ...
        changed_location = Location.objects.filter(name='Brazil')
